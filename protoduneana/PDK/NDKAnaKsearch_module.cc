...
public:
  ...
  void MCTruthInformation ( detinfo::DetectorClocksData const& clockData, const simb::MCParticle *particle );
    ...
    void truthMatcher( detinfo::DetectorClocksData const& clockData, 
		       ...
		       double &Efrac,
                       double &Ecomplet);
    void truthMatcher2( detinfo::DetectorClocksData const& clockData,
...
//========================================================================             
    ...
    std::vector<double> PFPStartMomentum;
    std::vector<double> PFPEndMomentum;
    std::vector<TVector3> PFPStartPoint;
    std::vector<TVector3> PFPEndPoint;
    std::vector<TVector3> PFPStartDirection;
    std::vector<int> PFP_Parent;
    std::vector<int> PFParticleID;
    std::vector<int> nDaughters;
    std::vector<int> nTrk_Cal;
    //std::vector<bool> Track_isContained;
...
//cout<<"End Process: "<<MC_Endprocess[i]<<endl;
 //      h_MC_end_Momentum->Fill(particle->EndPosition()(MC_endMomentum[i]));
  ...
  h_MC_length_Kaons->Fill(MC_truthlength[i]);
       ...
       std::cout << "Kaon Track Length " << MC_truthlength[i] << std::endl;
   //    std::cout << "Kaon Momentum "  <<particle->Momentum().E() << std::endl;
      // std::cout << "Start Pos. X, Y, Z " << particle->Position().X() <<", " << particle->Position().Y() <<", " <<  particle->Position().Z() << std::endl;
    //   std::cout << "End Pos.   X, Y, Z " << particle->EndPosition().X() <<", " << particle->EndPosition().Y() <<", " <<  particle->EndPosition().Z() << std::endl;
...
h_MC_DaughterMuons->Fill(MC_truthlength[i]);
h_MC_MotherKaons->Fill(MC_truthlength[i]);
...
int Pion =0;
  //this is a track base analysis so it must be at least one track
    ...
    //art::FindManyP<recob::Vertex> trk_from_vtx(trackListHandle,event,"pandora");
...
cout << "size of tracklist is " << tracklist.size() << ", size of trk_from_vtx is " << trk_from_vtx.size() << endl;
...
//changed 13 to 20 below
       ...
       //calculate PID
       cout << "size of Reco PID is " << reco_PID.size() << ", size of Reco Cal is " << reco_cal.size() << endl;
...
cout<<"Reco Track Lengths " << track->Length() << endl;
 ...
 //        std::cout <<"Reco PID 321" <<std::endl;
         std::cout <<"Reco Kaon Track Length " << track_length[i] << std::endl;
...
}else{h_track_length_NonWalls_Kaons->Fill(track->Length());}
...
}else{h_track_length_NonWalls_Muons->Fill(track->Length());}
...
//========================================================================
void NDKAna::truthMatcher(detinfo::DetectorClocksData const& clockData, 
                          std::vector<art::Ptr<recob::Hit>> all_hits,
                          std::vector<art::Ptr<recob::Hit>> track_hits, 
                          const simb::MCParticle*& MCparticle, 
                          double &Efrac, 
...
cout << "KANGAROO FRIENDS" <<endl;
   ...
   //      cout << "TrackIDs Size " <<TrackIDs.size() << endl;
       ...
       //     simb::MCParticle p = part_inv->TrackIdToParticle(k);
...
cout <<"truthMatcher2 Inside" <<endl;
	...
	for ( auto hit: track_hits.at(i) ) {
	    std::vector<sim::TrackIDE> trkIDEs = bt->HitToTrackIDEs(clockData,hit);
	    std::set<int> trkIDs;
