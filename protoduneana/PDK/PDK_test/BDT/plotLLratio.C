...
int plotLLratio(const char* fname, const char* templates_sig, const char* templates_bkg, const char* outfname) {
...
int EventSelector::Init()
{
    ...
    fHists["hNewTemplate"] = new TH2D("hNewTemplate",
				      ";Distance from vertex [cm];dE/dx",
				      50,0,50,50,0,50);
    fHists["hNewTemplate_rev"] = new TH2D("hNewTemplate_rev",
					  ";Distance from opposite end [cm];dE/dx",
					  50,0,50,50,0,50);
    ...
    for (auto it = fHists.begin(); it != fHists.end(); it++) {
...
int EventSelector::ProcessEvent(NDKAna* evt)
{
	...
	// loop over calorimetric points of the best plane
	 ...
	 double res_range_rev = evt->track_range[fShortIdx][rev_idx];
	 ...
	 fHists["hNewTemplate_rev"] -> Fill(res_range_rev, dEdx_rev);
	 ...
	 // 	 <<setw(15)<<"res range"<<setw(15)<<res_range<<endl
	 // 	 <<setw(15)<<"rev range"<<setw(15)<<res_range_rev<<endl
...
int EventSelector::Finalise()
{
    ...
    c->SaveAs(name+"_dedx_vs_resrange.pdf");
