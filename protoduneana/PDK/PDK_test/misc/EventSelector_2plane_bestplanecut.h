...
int EventSelector::Loop(unsigned nentries)
{
	...
	int bestplane_npoints = fEvent->n_cal_points[fShortIdx];

	// pick 2 planes with most hits
	if ( fEvent->n_cal_points_byplane[fShortIdx][0] > fEvent->n_cal_points_byplane[fShortIdx][1] ) {
	    fBestTwoPlanes[0] = 0;
	    fBestTwoPlanes[1] = 1;
	} else {
	    fBestTwoPlanes[0] = 1;
	...
	if ( fEvent->n_cal_points_byplane[fShortIdx][2] > fBestTwoPlanes[1] )
	    fBestTwoPlanes[1] = 2;
	...
	if (bestplane_npoints < fNpointCut) continue;
