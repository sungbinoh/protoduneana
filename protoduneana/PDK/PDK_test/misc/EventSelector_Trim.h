...
int EventSelector::Loop(unsigned nentries)
{
	...
	//     <<" event: "<<fEvent->eventNo<<endl;

	// int position = 0;

	// make sure all variable array sizes are within their max range
	if (fEvent->mcgenie_npart > MAX_GEN_PARTS) fEvent->mcgenie_npart = MAX_GEN_PARTS;
	if (fEvent->mc_npart > MAX_GEN_PARTS) fEvent->mc_npart = MAX_MC_PARTS;
	if (fEvent->n_vertices > MAX_RECO_VERTICES) fEvent->n_vertices = MAX_RECO_VERTICES;
	if (fEvent->n_reco_tracks > MAX_RECO_TRACKS) fEvent->n_reco_tracks = MAX_RECO_TRACKS;
	if (fEvent->n_decayVtx > MAX_RECO_VERTICES) fEvent->n_decayVtx = MAX_RECO_VERTICES;
	//if (fEvent->n_cal_points > 500) fEvent->n_cal_points = 500;
	if (fEvent->n_showers > MAX_SHOWERS) fEvent->n_showers = MAX_SHOWERS;
	if (fEvent->n_flashes > MAX_FLASHES) fEvent->n_flashes = MAX_FLASHES;
	...
	//if (npoints <= 0 || npoints > MAX_CALO_PTS) continue;
