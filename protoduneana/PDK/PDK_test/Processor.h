...
public:
    Processor(const char* fname, const char* outfname);
...
void Processor::AllowBranches(const std::vector<string>& br_list)
{
...
int Processor::Loop(unsigned nentries)
{
    Init();
	...
	FindLongestShortest(fEvent, fShortIdx, fLongIdx);
	...
	if(  (fCutMask & (1<<kNoLongProton)) &&  abs(fEvent->track_mcPDG[fLongIdx]) == 2212 )
	    continue;
	cuts_stats[kNoLongProton]++;
	//short track cannot be a pion or lepton
	if( (fCutMask & (1<<kNoShortPion)) && abs(fEvent->track_mcPDG[fShortIdx]) < 321 )
	    continue;
	...
	int npoints = fEvent->n_cal_points[fShortIdx];

	// check ordering of calo points: 1 - last in track, first in calo points; 0 - oposite
	if (npoints > 1)
	    fRangeReversed = (fEvent->track_range[fShortIdx][0] <
			      fEvent->track_range[fShortIdx][npoints-1]);
	else
	    fRangeReversed = -1;
...
int Processor::FindLongestShortest(NDKAna* signal, int& shortest_trk_idx, int& longest_trk_idx) {
...
int Processor::GetDirectionOfTracks(NDKAna* signal,
					 int shortest_trk_idx, int longest_trk_idx,
					 int& sh_trk_dir, int& lo_trk_dir,
					 int& common_vtx)
{
...
void Processor::printStats (unsigned* stats)
{
    // const int width = 15;
