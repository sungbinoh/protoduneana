  ...
  : EDFilter{p}  // ,
{
  ...
  _fidVolOuter.Configure(pOuterFV, thisFullDetActiveVol);
  _fidVolOuter.Configure(pTpcFV, thisActiveVol);
  _fidVolOuter.PrintConfiguration();
  _fidVolInner.Configure(pInnerFV, thisFullDetActiveVol);
  ...
  for ( size_t i = 0; i < pfpPtrVector.size(); i++){
    ...
    if (thisPfp->PdgCode() == 13){
      ...
      if (  !_fidVolOuter.IsInDetectorFV(thisTrack->Start().X(), thisTrack->Start().Y(), thisTrack->Start().Z()) 
        ...
        // get calorimetry information, but only for root tree
        ...
        for (size_t i_cal = 0; i_cal < theseCalo.size(); i_cal++){
          ...
          art::Ptr< anab::Calorimetry > calrimetryForCollectionPtr = makeCalorimetryPtr(calorimetryCollection->size() -1);
          calorimetryPtrCollection.push_back(calrimetryForCollectionPtr);
...
{  
  trackStartXZAllTracks = tfs->make<TH2D>("trackStartXZAllTracks", ";Track Start Z (cm);Track Start X (cm)", 100, -100, 800, 100, -450, 450);
  trackEndXZAllTracks   = tfs->make<TH2D>("trackEndXZAllTracks", ";Track End Z (cm);Track End X (cm)", 100, -100, 800, 100, -450, 450);
  trackStartYZAllTracks = tfs->make<TH2D>("trackStartYZAllTracks", ";Track Start Z (cm);Track Start Y (cm)", 100, -100, 800, 100, -100, 800);
  trackEndYZAllTracks   = tfs->make<TH2D>("trackEndYZAllTracks", ";Track End Z (cm);Track End Y (cm)", 100, -100, 800, 100, -100, 800);
