#define protoDUNE_validate_calib_Birks_cxx
#include "protoDUNE_validate_calib_Birks.h"
#include <TH2.h>
#include <TH3.h>
#include <TStyle.h>
#include <TCanvas.h>
...
float GetdEdx_Birks(float dQdx, float E_field){
  ...
  return( dQdx / ( (A_B/Wion) - k_B * (dQdx /(Rho * E_field) )) );
}
...
void protoDUNE_validate_calib_Birks::Loop(int mn)
{
  //cout << "SB debug [protoDUNE_validate_calib_Birks::Loop] loop starts" << endl;
  if (fChain == 0) return;
  fChain->GetEntry(0);
  if (run>10000) run = 0;
  ...
  if (recalib){
    //cout << "SB debug [protoDUNE_validate_calib_Birks::Loop] recalib" << endl;
    ifstream in;
    for (int i = 0; i<3; ++i){
      in.open(Form("global_median_%d_r%d_%s.txt",i, run, ref_dedx_str.c_str()));
      char line[1024];
      while(1){
      ...
      in.close();
      in.clear();
      in.open(Form("./calconsts/calconst_%d_%s.txt", i, ref_dedx_str.c_str()));
      while(1){
        in.getline(line,1024);
        if(!in.good()) break;
        double error;
    ...
    my_file2 = TFile::Open(Form("Xcalo_mich%d_r%d_%s.root",mn, run, ref_dedx_str.c_str()));
        ...
        if ((fid_xing || stpok) && (negok || posok)){
		  ...
		  //cout << "SB debug, " << jentry << "'th event, (i,j,k) : (" << i << ", " << j << ", " << k << "), dqdx : " << corrected_dqdx << ", dedx : " << corrected_dedx;
                  if (recalib){  
                    ...
                    corrected_dedx=GetdEdx_Birks(corrected_dqdx, tot_Ef(trkhitx[i][j][k],trkhity[i][j][k],trkhitz[i][j][k]));
                  }
                  dQdx = corrected_dqdx;
...
int main(int argc, char *argv[]) {
  ...
  ref_dedx_str  = pset.get<string>("ref_dedx");
  ...
  cout<<"ref_dedx = "<<ref_dedx_str<<endl;
  ...
  if(ref_dedx > 1.95 && ref_dedx < 2.05) ref_dedx_str = "2.0";
  ...
  //shtree -> Print();
  protoDUNE_validate_calib_Birks t(shtree);
